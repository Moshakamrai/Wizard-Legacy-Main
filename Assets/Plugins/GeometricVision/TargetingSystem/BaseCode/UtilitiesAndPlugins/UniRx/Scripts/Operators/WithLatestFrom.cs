    ...
    internal class WithLatestFromObservable<TLeft, TRight, TResult> : OperatorObservableBase<TResult>
    {
        ...
        class WithLatestFrom : OperatorObserverBase<TResult, TResult>
        {
            ...
            public IDisposable Run()
            {
                ...
                rSubscription.Disposable  = this.parent.right.Subscribe(new RightObserver(this, rSubscription));
            ...
            class LeftObserver : IObserver<TLeft>
            {
                ...
                public void OnNext(TLeft value)
                {
                    if (this.parent.hasLatest)
                    {
                        ...
                        try
                        {
                            result = this.parent.parent.selector(value, this.parent.latestValue);
                        }
                        catch (Exception ex)
                        {
