...
namespace Plugins.GeometricVision.TargetingSystem.Code.UtilitiesAndPlugins.UniRx.Scripts.UnityEngineBridge
{
    ...
    public class ReactiveCommand : ReactiveCommand<Unit>
    {
        ...
        public bool Execute()
        {
            return this.Execute(Unit.Default);
        }

        /// <summary>Force push parameter to subscribers.</summary>
        public void ForceExecute()
        {
            this.ForceExecute(Unit.Default);
        }
    }
    ...
    public class ReactiveCommand<T> : IReactiveCommand<T>, IDisposable
    {
        ...
        public ReactiveCommand(IObservable<bool> canExecuteSource, bool initialValue = true)
        {
            ...
            this.canExecuteSubscription = canExecuteSource
                ...
                .SubscribeWithState(this.canExecute, (b, c) => c.Value = b);
        }
    ...
    public class AsyncReactiveCommand<T> : IAsyncReactiveCommand<T>
    {
        ...
        public IDisposable Execute(T parameter)
        {
            if (this.canExecute.Value)
            {
                ...
                if (a.Length == 1)
                {
                    try
                    {
                        var asyncState = a[0].Invoke(parameter) ?? Scripts.Observable.ReturnUnit();
                        return asyncState.Finally(() => this.canExecuteSource.Value = true).Subscribe();
                    }
                    catch
                    {
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        {
                            xs[i] = a[i].Invoke(parameter) ?? Scripts.Observable.ReturnUnit();
                        }
                    }
                    catch
                    {
                    ...
                    return Scripts.Observable.WhenAll(xs).Finally(() => this.canExecuteSource.Value = true).Subscribe();
                }
            }
            else
            {
        ...
        public void Dispose()
        {
            ...
            this.IsDisposed = true;
            this.asyncActions = ImmutableList<Func<T, IObservable<Unit>>>.Empty;
        }
    ...
    public static class ReactiveCommandExtensions
    {
        ...
        public static global::System.Runtime.CompilerServices.TaskAwaiter<T> GetAwaiter<T>(this IReactiveCommand<T> command)
        {
    ...
    public static class AsyncReactiveCommandExtensions
    {
        ...
        public static Task<T> WaitUntilExecuteAsync<T>(this IAsyncReactiveCommand<T> source, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            var subscription = source.Subscribe(x => { tcs.TrySetResult(x); return Scripts.Observable.ReturnUnit(); });
            cancellationToken.Register(Callback, Tuple.Create(tcs, subscription), false);

            return tcs.Task;
        }

        public static global::System.Runtime.CompilerServices.TaskAwaiter<T> GetAwaiter<T>(this IAsyncReactiveCommand<T> command)
        {
            return command.WaitUntilExecuteAsync(CancellationToken.None).GetAwaiter();
        }
