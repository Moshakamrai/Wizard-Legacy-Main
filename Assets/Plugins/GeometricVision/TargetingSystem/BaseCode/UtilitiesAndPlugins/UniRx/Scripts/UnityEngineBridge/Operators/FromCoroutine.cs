...
namespace Plugins.GeometricVision.TargetingSystem.Code.UtilitiesAndPlugins.UniRx.Scripts.UnityEngineBridge.Operators
{
    internal class FromCoroutineObservable<T> : OperatorObservableBase<T>
    {
        ...
        protected override IDisposable SubscribeCore(IObserver<T> observer, IDisposable cancel)
        {
            ...
            MainThreadDispatcher.SendStartCoroutine(this.coroutine(fromCoroutineObserver, token));
        ...
        protected override IDisposable SubscribeCore(IObserver<T> observer, IDisposable cancel)
        {
            ...
            switch (this.frameCountType)
            {
                case FrameCountType.Update:
                    MainThreadDispatcher.StartUpdateMicroCoroutine(this.coroutine(microCoroutineObserver, token));
                    break;
                case FrameCountType.FixedUpdate:
                    MainThreadDispatcher.StartFixedUpdateMicroCoroutine(this.coroutine(microCoroutineObserver, token));
                    break;
                case FrameCountType.EndOfFrame:
                    MainThreadDispatcher.StartEndOfFrameMicroCoroutine(this.coroutine(microCoroutineObserver, token));
                    break;
                default:
