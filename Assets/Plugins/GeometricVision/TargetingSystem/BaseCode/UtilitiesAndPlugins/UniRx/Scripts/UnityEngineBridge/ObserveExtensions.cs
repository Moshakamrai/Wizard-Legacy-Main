...
#if !UniRxLibrary
using ObservableUnity = Plugins.GeometricVision.TargetingSystem.Code.UtilitiesAndPlugins.UniRx.Scripts.Observable;
#endif
...
namespace Plugins.GeometricVision.TargetingSystem.Code.UtilitiesAndPlugins.UniRx.Scripts.UnityEngineBridge
{
    public static partial class ObserveExtensions
    {
        ...
        public static IObservable<TProperty> ObserveEveryValueChanged<TSource, TProperty>(this TSource source, Func<TSource, TProperty> propertySelector, FrameCountType frameCountType, IEqualityComparer<TProperty> comparer, bool fastDestroyCheck)
            where TSource : class
        {
            if (source == null) return ObservableUnity.Empty<TProperty>();
            ...
            if (isUnityObject && unityObject == null) return ObservableUnity.Empty<TProperty>();
