...
namespace Plugins.GeometricVision.TargetingSystem.Code.UtilitiesAndPlugins.UniRx.Scripts.UnityEngineBridge
{
    ...
    public class InspectorDisplayDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            {
                this.EmitPropertyField(position, targetSerializedProperty, label);
            }
            ...
            if (notifyPropertyChanged)
            {
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    var targetProp = (paths.Length == 1)
                        ? this.fieldInfo.GetValue(attachedComponent)
                        : this.GetValueRecursive(attachedComponent, 0, paths);
        ...
        object GetValueRecursive(object obj, int index, string[] paths)
        {
            ...
            if (fldInfo == null && path == "Array")
            {
                try
                {
                    ...
                    var arrayValue = (obj as global::System.Collections.IList)[arrayIndex];
                    if (index < paths.Length - 1)
                    {
                        return this.GetValueRecursive(arrayValue, ++index, paths);
                    }
                    else
                    {
                        return arrayValue;
            ...
            if (index < paths.Length - 1)
            {
                return this.GetValueRecursive(v, ++index, paths);
            }
        ...
        public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
        {
            ...
            {
                var multilineAttr = this.GetMultilineAttribute();
        ...
        {
            var multiline = this.GetMultilineAttribute();
            if (multiline == null)
            {
                var range = this.GetRangeAttribute();
                if (range == null)
                {
